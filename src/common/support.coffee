...
support =
  ...
  normalizeAlbumName: (value) ->
    ...
    value = value.replace(/^([\w\s]+\w)\-[A-Z0-9][A-Z0-9]+.+$/g, "$1") # Album Name-CATNO Vinyl
    [
  ...
  normalizeArtistName: (value) ->
    support.normalize(value.replace(/\b(ft|feat|presents)[\_\-\s\.]+.+$/i, ''))
